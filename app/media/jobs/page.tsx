...
export default function JobsPage() {
  ...
  useEffect(() => {
    async function fetchJobs() {
      try {
        const data = (await careersAPI.getAll()) as Job[];
        setJobs(data);
      } catch (error) {
      ...
      <section className="bg-gradient-to-r from-primary-500 to-secondary-500 text-white section-py">
        <div className="container-page text-center">
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="section-subtitle text-white/90">
            انضم إلى فريقنا وساهم في إحداث التغيير
          </motion.p>
        </div>
      </section>
          ...
          <div className="space-y-4">
            {jobs.map((job, i) => (
              <motion.div
                ...
                className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 flex flex-wrap justify-between items-center"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-4 mt-1 text-sm text-gray-500">
                      ...
                      <span className="flex items-center gap-1">
                        <FaCalendar /> {formatDate(job.deadline)}
                      </span>
                ...
                <Link
                  href={`/media/jobs/${job.id}`}
                  className="mt-4 md:mt-0 px-6 py-2 bg-primary-500 text-white rounded-xl hover:bg-primary-600 inline-flex items-center gap-2"
                >
