import Image from 'next/image';
import Link from 'next/link';
import { notFound } from 'next/navigation';
import { FaArrowLeft, FaFolder, FaImages, FaMapPin, FaQuoteRight, FaStar, FaUser } from 'react-icons/fa6';

import MediaGallery from '@/components/content/MediaGallery';
import RichContent from '@/components/content/RichContent';
import { entity, fetchJSON } from '@/lib/api/server';
import { galleryImages, primaryImage } from '@/lib/format';
import { extractHashtags } from '@/lib/rich-text';

export const revalidate = 900;

type StoryProgram = {
  id?: number | string;
  name?: string;
  name_ar?: string;
};

type StoryDetail = {
  content?: string | null;
  published_at?: string;
  featured?: boolean;
  gallery?: string[] | null;
  id: number | string;
  image?: string | null;
  location?: string | null;
  person_name?: string | null;
  program?: StoryProgram | null;
  title?: string;
  title_ar?: string;
};

async function getStory(id: string): Promise<StoryDetail | null> {
  const payload = await fetchJSON<StoryDetail | { data?: StoryDetail }>(`/success-stories/${id}`, 900);
  return entity<StoryDetail>(payload, 'story');
}

export default async function SuccessStoryDetailPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const story = await getStory(id);

  if (!story) {
    notFound();
  }

  const title = story.title_ar || story.title || 'قصة نجاح';
  const cover = primaryImage(story.image, story.gallery);
  const gallery = story.image ? galleryImages(story.gallery) : galleryImages(story.gallery).slice(1);
  const tags = extractHashtags(story.content);
  const programName = story.program?.name_ar || story.program?.name || 'غير مصنف';

  return (
    <div className="pt-navbar bg-gray-50 dark:bg-gray-950">
      <section className="overflow-hidden bg-[radial-gradient(circle_at_top_right,_rgba(255,255,255,0.18),_transparent_35%),linear-gradient(135deg,#D4621A_0%,#159C4B_100%)] text-white">
        <div className="container-page py-12 md:py-16">
          <Link href="/media/success-stories" className="mb-6 inline-flex items-center gap-2 text-sm font-semibold text-white/80 hover:text-white">
            <FaArrowLeft />
            العودة إلى قصص النجاح
          </Link>

          <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
            <div>
              <div className="mb-5 flex flex-wrap gap-3">
                {story.featured ? (
                  <span className="inline-flex items-center gap-2 rounded-full bg-white/15 px-4 py-2 text-sm font-semibold text-white backdrop-blur">
                    <FaStar className="text-yellow-300" />
                    قصة مميزة
                  </span>
                ) : null}
                <span className="inline-flex items-center gap-2 rounded-full bg-white/15 px-4 py-2 text-sm font-semibold text-white backdrop-blur">
                  <FaFolder />
                  {programName}
                </span>
              </div>

              <h1 className="max-w-4xl text-3xl font-bold leading-tight md:text-5xl">{title}</h1>

              <div className="mt-8 flex flex-wrap gap-3 text-sm text-white/85">
                <span className="inline-flex items-center gap-2 rounded-full bg-black/15 px-4 py-2 backdrop-blur">
                  <FaUser />
                  {story.person_name || 'مستفيد'}
                </span>
                <span className="inline-flex items-center gap-2 rounded-full bg-black/15 px-4 py-2 backdrop-blur">
                  <FaMapPin />
                  {story.location || 'غير محدد'}
                </span>
                <span className="inline-flex items-center gap-2 rounded-full bg-black/15 px-4 py-2 backdrop-blur">
                  <FaImages />
                  {gallery.length} صور إضافية
                </span>
              </div>
            </div>

            <div className="relative overflow-hidden rounded-[2rem] border border-white/10 shadow-2xl">
              <div className="relative aspect-[4/3]">
                <Image src={cover} alt={title} fill className="object-cover" sizes="(max-width: 1024px) 100vw, 40vw" priority />
              </div>
            </div>
          </div>
        </div>
      </section>

      <section className="section-py">
        <div className="container-page grid gap-8 xl:grid-cols-[minmax(0,1fr)_320px]">
          <article className="space-y-8">
            <div className="rounded-[2rem] border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900 md:p-8">
              <div className="mb-6 flex items-center gap-3">
                <span className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-primary-50 text-primary-600 dark:bg-primary-900/20 dark:text-primary-300">
                  <FaQuoteRight className="text-lg" />
                </span>
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-white">القصة كاملة</h2>
                  <p className="text-sm text-gray-500 dark:text-gray-400">عرض منسق للمحتوى مع الروابط والوسوم والصور</p>
                </div>
              </div>
              <RichContent content={story.content || ''} />
            </div>

            <MediaGallery images={gallery} title={title} />
          </article>

          <aside className="space-y-6 xl:sticky xl:top-28 xl:self-start">
            <div className="rounded-[2rem] border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
              <h3 className="mb-4 text-lg font-bold text-gray-900 dark:text-white">بطاقة المستفيد</h3>
              <div className="space-y-3 text-sm text-gray-600 dark:text-gray-300">
                <p className="flex items-center gap-3 rounded-2xl bg-gray-50 px-4 py-3 dark:bg-gray-800/70">
                  <FaUser className="text-primary-500" />
                  <span>{story.person_name || 'غير محدد'}</span>
                </p>
                <p className="flex items-center gap-3 rounded-2xl bg-gray-50 px-4 py-3 dark:bg-gray-800/70">
                  <FaMapPin className="text-primary-500" />
                  <span>{story.location || 'غير محدد'}</span>
                </p>
                <p className="flex items-center gap-3 rounded-2xl bg-gray-50 px-4 py-3 dark:bg-gray-800/70">
                  <FaFolder className="text-primary-500" />
                  <span>{programName}</span>
                </p>
              </div>
            </div>

            {tags.length > 0 ? (
              <div className="rounded-[2rem] border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <h3 className="mb-4 text-lg font-bold text-gray-900 dark:text-white">وسوم القصة</h3>
                <div className="flex flex-wrap gap-2">
                  {tags.map((tag) => (
                    <span
                      key={tag}
                      className="inline-flex rounded-full bg-primary-50 px-3 py-1.5 text-sm font-semibold text-primary-700 ring-1 ring-primary-100 dark:bg-primary-900/20 dark:text-primary-300 dark:ring-primary-800/60"
                    >
                      {tag}
                    </span>
                  ))}
                </div>
              </div>
            ) : null}
          </aside>
        </div>
      </section>
    </div>
  );
}
