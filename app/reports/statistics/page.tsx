...
export default function StatisticsPage() {
  const [summary, setSummary] = useState<any>(fallbackSummary);
  ...
  useEffect(() => {
    async function fetchStatistics() {
      try {
        ...
        setSummary({
          counts: { ...fallbackSummary.counts, ...(summaryData?.counts || {}) },
          totals: { ...fallbackSummary.totals, ...(summaryData?.totals || {}) },
          recent_activity: { ...fallbackSummary.recent_activity, ...(summaryData?.recent_activity || {}) },
        });
  ...
  const mainStats = useMemo(() => {
    ...
    return [
      { label: 'المشاريع', value: counts.projects, icon: FaFolderOpen, color: '#1A5F7A', detail: 'مشروع ومبادرة' },
      { label: 'المستفيدون', value: totals.beneficiaries, icon: FaPeopleGroup, color: '#159C4B', detail: 'إجمالي الأفراد' },
      { label: 'الأسر', value: totals.families, icon: FaUsers, color: '#D4621A', detail: 'أسرة مستفيدة' },
      { label: 'المتطوعون', value: counts.volunteers, icon: FaHandHoldingHeart, color: '#8B5CF6', detail: 'طلب ومشاركة' },
    ];
  }, [summary]);
  ...
  const contentStats = useMemo(() => {
    ...
    return [
      { label: 'الأخبار', value: counts.news, icon: FaNewspaper, color: '#2563EB' },
      { label: 'قصص النجاح', value: counts.success_stories, icon: FaStar, color: '#F59E0B' },
      { label: 'مشاهدات الأخبار', value: totals.news_views, icon: FaEye, color: '#EF4444' },
      { label: 'مشاهدات الفيديو', value: totals.video_views, icon: FaChartLine, color: '#0EA5E9' },
    ];
  }, [summary]);
  ...
  const actionStats = useMemo(() => {
    ...
    return [
      { label: 'مشتركون نشطون', value: counts.subscribers, icon: FaEnvelope, color: '#159C4B' },
      { label: 'مناقصات مفتوحة', value: counts.open_tenders, icon: FaBullhorn, color: '#D4621A' },
      { label: 'وظائف مفتوحة', value: counts.open_careers, icon: FaBriefcase, color: '#1A5F7A' },
    ];
  }, [summary]);

  const maxContentValue = Math.max(1, ...contentStats.map((item) => Number(item.value || 0)));
            ...
            {mainStats.map((item, index) => (
              ...
              >
                <div className="flex items-center justify-between gap-4 mb-5">
                  <span className="w-14 h-14 rounded-2xl flex items-center justify-center" style={{ backgroundColor: `${item.color}20` }}>
            ...
            <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-100 dark:border-gray-700">
              ...
              <div className="space-y-5">
                {contentStats.map((item) => {
                  const width = `${Math.max(8, (Number(item.value || 0) / maxContentValue) * 100)}%`;

                  return (
                    <div key={item.label}>
                      <div className="flex items-center justify-between gap-4 mb-2">
                        <div className="flex items-center gap-3">
                          <span className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: `${item.color}20` }}>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-100 dark:border-gray-700">
              ...
              <div className="space-y-4">
                {actionStats.map((item) => (
                  <div key={item.label} className="flex items-center justify-between rounded-xl bg-gray-50 dark:bg-gray-700/60 p-4">
                    <div className="flex items-center gap-3">
                      <span className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: `${item.color}20` }}>
                        <item.icon style={{ color: item.color }} />
                      </span>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-100 dark:border-gray-700">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {homeStats.map((stat) => {
                  ...
                  return (
                    <div key={stat.key || stat.label_ar} className="rounded-xl bg-gray-50 dark:bg-gray-700/60 p-4">
                      <div className="flex items-center gap-3 mb-3">
                        <span className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: `${stat.color || '#1A5F7A'}20` }}>
                          <Icon style={{ color: stat.color || '#1A5F7A' }} />
                        </span>
                        <span className="text-sm text-gray-500 dark:text-gray-400">{stat.label_ar}</span>
                      </div>
